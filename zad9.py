...
class FuzzyPlayer(Player):
    def __init__(self, racket: Racket, ball: Ball, board: Board):
        ...
        self.racket.max_speed = 30

        self.x_dist = fuzzcontrol.Antecedent(np.arange(-800, 801, 1), 'x_dist')
        self.y_dist = fuzzcontrol.Antecedent(np.arange(0, 601, 1), 'y_dist')
        self.velocity = fuzzcontrol.Consequent(np.arange(-30, 31, 1), 'velocity')

        self.x_dist['far_left'] = fuzz.trapmf(self.x_dist.universe, [-800, -800, -100, -50])
        self.x_dist['left'] = fuzz.trimf(self.x_dist.universe, [-60, -30, 0])
        self.x_dist['center'] = fuzz.trimf(self.x_dist.universe, [-10, 0, 10])
        self.x_dist['right'] = fuzz.trimf(self.x_dist.universe, [0, 30, 60])
        self.x_dist['far_right'] = fuzz.trapmf(self.x_dist.universe, [50, 100, 800, 800])

        self.y_dist['near'] = fuzz.trimf(self.y_dist.universe, [0, 0, 150])
        self.y_dist['mid'] = fuzz.trimf(self.y_dist.universe, [100, 200, 350])
        self.y_dist['far'] = fuzz.trimf(self.y_dist.universe, [300, 600, 600])

        self.velocity['left_fast'] = fuzz.trapmf(self.velocity.universe, [-30, -30, -20, -15])
        self.velocity['left_slow'] = fuzz.trimf(self.velocity.universe, [-15, -8, 0])
        self.velocity['stay'] = fuzz.trimf(self.velocity.universe, [-5, 0, 5])
        self.velocity['right_slow'] = fuzz.trimf(self.velocity.universe, [0, 8, 15])
        self.velocity['right_fast'] = fuzz.trapmf(self.velocity.universe, [15, 20, 30, 30])

        rules = [
            fuzzcontrol.Rule(self.x_dist['far_left'], self.velocity['right_fast']),
            fuzzcontrol.Rule(self.x_dist['far_right'], self.velocity['left_fast']),

            fuzzcontrol.Rule(self.x_dist['left'] & self.y_dist['near'], self.velocity['right_fast']),
            fuzzcontrol.Rule(self.x_dist['right'] & self.y_dist['near'], self.velocity['left_fast']),

            fuzzcontrol.Rule(self.x_dist['left'] & self.y_dist['mid'], self.velocity['right_fast']),
            fuzzcontrol.Rule(self.x_dist['right'] & self.y_dist['mid'], self.velocity['left_fast']),

            fuzzcontrol.Rule(self.x_dist['left'] & self.y_dist['far'], self.velocity['right_slow']),
            fuzzcontrol.Rule(self.x_dist['right'] & self.y_dist['far'], self.velocity['left_slow']),

            fuzzcontrol.Rule(self.x_dist['center'], self.velocity['stay']),
        ]

        system = fuzzcontrol.ControlSystem(rules)
        self.racket_controller = fuzzcontrol.ControlSystemSimulation(system)

    def act(self, x_diff: int, y_diff: int):
        velocity = int(self.make_decision(x_diff, y_diff))
        target_x = self.racket.rect.x + velocity
        self.racket.move(target_x, self.board)

    def make_decision(self, x_diff: int, y_diff: int):
        x_diff = np.clip(x_diff, -800, 800)
        y_diff = np.clip(y_diff, 0, 600)

        self.racket_controller.input['x_dist'] = x_diff
        self.racket_controller.input['y_dist'] = y_diff
        self.racket_controller.compute()

        return self.racket_controller.output.get('velocity', 0)
